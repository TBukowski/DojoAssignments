#USE world;
#1) get all the countries that speak Slovene? Your query should return
#the name of the country, language and language percentage. 
...
#2) display the total number of cities for each country? Your query should return 
#the name of the country and the total number of cities. Your query should arrange 
#the result by the number of cities in descending order.
...
FROM countries
WHERE surface_area < 581 AND population > 100000;
*/
...
SELECT name, life_expectancy, capital 
FROM countries
...
#7) get all the cities of Argentina inside the Buenos Aires district and have 
#the population greater than 500, 000? The query should return the Country Name, 
...
#8) summarize the number of countries in each region? The query should  
# displaythe name of the region and the number of countries. Also, the query should 
# arrange the result by the number of countries in descending order. 
/*
SELECT region, COUNT(id) total_countries
FROM countries
GROUP BY region
